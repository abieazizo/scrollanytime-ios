...
func handleSubscribeTouch(message: WKScriptMessage) {
  // [START subscribe_topic]
    ...
    if (subscribeMessages.count > 0){
        ...
        // Topic subscription was Firebase's; without it there is nothing to subscribe to. The
        // web app never sends this message in the iOS shell (push is hidden there), so this is
        // unreachable — it stays only so the WKScriptMessage contract is still total.
        _ = _message
    }
...
/// There is no push provider in this build, so there is no token. Answer the web app honestly
/// rather than leaving its promise unresolved.
func handleFCMToken(){
    DispatchQueue.main.async(execute: {
